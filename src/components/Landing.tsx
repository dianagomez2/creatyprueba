...
function Hero() {
  ...
  return <section id="top" className="relative flex min-h-[780px] items-center overflow-hidden bg-creaty-black px-5 pb-24 pt-36 text-creaty-cream md:min-h-[860px] md:px-10 md:pt-44"><div className="hero-grid absolute inset-0 opacity-40"/><div className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-creaty-lime/50 to-transparent"/><motion.div initial={reduce?false:{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{duration:.8}} className="relative mx-auto max-w-5xl text-center"><h1 className="type-hero mx-auto font-black tracking-normal">Más emprendimientos acompañados. <span className="text-creaty-lime">Menos tiempo</span> preparando cada mentoría.</h1><p className="type-support mx-auto mt-6 text-creaty-cream/70">Creaty centraliza los avances de tus emprendedores y te ayuda a llegar a cada sesión con el contexto y los próximos pasos claros, para que dediques tu tiempo a aportar valor.</p><div className="mt-8 flex flex-col items-center"><CTAButton>Regístrate ahora</CTAButton><span className="mt-3 text-xs text-creaty-cream/45">Y recibe gratis un diagnóstico como mentor para iniciar</span></div></motion.div></section>;
}

function DashboardSection(){return <Section light><div className="mx-auto max-w-4xl text-center"><p className="text-xs font-bold uppercase text-creaty-orange">Un solo lugar para acompañar mejor</p><h2 className="type-section mx-auto mt-3 font-black">Todo lo que necesitas saber antes de tu próxima mentoría.</h2><p className="type-support mx-auto mt-4 text-creaty-black/60">Avances, acuerdos, próximos pasos y recomendaciones de cada emprendimiento, organizados en un solo lugar.</p></div><h3 className="type-card-title mx-auto mt-12 max-w-3xl text-center font-extrabold">Llega con contexto. Empieza aportando valor desde el primer minuto.</h3><DashboardMockup/></Section>}
...
function Problema(){return <Section id="problema"><div className="max-w-4xl"><p className="text-xs font-bold uppercase text-creaty-orange">Por qué Creaty</p><h2 className="type-section mt-3 font-black">Tu tiempo debería estar dedicado a mentorear, no reconstruyendo lo que pasó.</h2><p className="type-support mt-4 text-creaty-cream/60">Entre una sesión y otra, el contexto se dispersa, los compromisos se enfrían y preparar cada caso vuelve a consumir tiempo.</p></div><div className="mt-10 grid gap-px overflow-hidden rounded-lg border border-creaty-cream/10 bg-creaty-cream/10 md:grid-cols-2">{problems.map(([t,d],i)=><article key={t} className="bg-creaty-black p-6 md:p-8"><span className={`text-3xl font-black ${i%2?"text-creaty-purple":"text-creaty-orange"}`}>0{i+1}</span><h3 className="type-card-title mt-4 font-bold">{t}</h3><p className="mt-3 text-sm leading-relaxed text-creaty-cream/55">{d}</p></article>)}</div><div className="mt-12 flex flex-col items-start justify-between gap-5 border-t border-creaty-cream/10 pt-8 md:flex-row md:items-center"><p className="max-w-xl font-semibold">Descubre qué puedes mejorar en tu forma de acompañar y recibe gratis tu diagnóstico como mentor.</p><CTAButton>Haz tu diagnóstico gratis</CTAButton></div></Section>}
...
function ProcessMap(){return <div className="relative mx-auto mt-12">
...
function Solucion(){return <Section id="solucion" light><div className="mx-auto max-w-4xl text-center"><p className="text-xs font-bold uppercase text-creaty-orange">Todo lo que necesitas para un acompañamiento de verdad</p><h2 className="type-section mx-auto mt-3 font-black">Tu experiencia hace la mentoría. Creaty hace que llegues preparado.</h2><p className="type-support mx-auto mt-4 text-creaty-black/60">Con Creaty conectas lo que ocurrió, lo que avanzó y lo que debería seguir para que tú puedas concentrarte en preguntar, analizar y orientar a tus emprendedores.</p></div><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{values.map(([Icon,t,d,c])=><article key={t} className="rounded-lg border border-creaty-black/10 bg-background p-5"><div className={`grid h-11 w-11 place-items-center rounded-md ${c}`}><Icon className="h-5 w-5"/></div><h3 className="type-card-title mt-5 font-extrabold">{t}</h3><p className="mt-3 text-sm leading-relaxed text-creaty-black/55">{d}</p></article>)}</div><ProcessMap/></Section>}
...
function FinalCTA(){return <Section><div className="mx-auto max-w-4xl text-center"><Target className="mx-auto h-10 w-10 text-creaty-orange"/><h2 className="type-section mx-auto mt-5 font-black">Tu siguiente mentoría empieza mucho antes de la sesión.</h2><p className="type-support mx-auto mt-4 text-creaty-cream/65">Regístrate, recibe gratis tu diagnóstico como mentor y sé de los primeros en conocer cómo Creaty puede ayudarte a acompañar más emprendimientos dedicando menos tiempo operativo a preparar cada sesión.</p><div className="mt-8"><CTAButton>Regístrate ahora</CTAButton></div></div></Section>}
