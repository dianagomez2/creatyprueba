...
function Logo({ compact = false }: { compact?: boolean }) {
  return <a href="#top" aria-label="Ir al inicio" className="inline-flex shrink-0 items-center"><img src={creatyLogo} alt="Creaty" className={`${compact ? "h-8 md:h-11 lg:h-12" : "h-14 md:h-[72px]"} w-auto object-contain`} /></a>;
}
...
function Navbar() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-[100] border-b transition-all duration-300 ${scrolled || menuOpen ? "border-creaty-cream/10 bg-creaty-black/95 shadow-[var(--shadow-nav)] backdrop-blur-xl" : "border-transparent bg-transparent"}`}>
    <div className="site-container relative grid min-h-16 lg:min-h-[72px] grid-cols-[1fr_auto] items-center gap-3 py-2 md:grid-cols-[1fr_auto_1fr]">
