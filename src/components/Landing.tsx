...
function Navbar() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-[100] border-b transition-all duration-300 ${scrolled || menuOpen ? "border-creaty-cream/10 bg-creaty-black/95 shadow-[var(--shadow-nav)] backdrop-blur-xl" : "border-transparent bg-transparent"}`}>
    <div className="site-container relative grid min-h-16 grid-cols-[1fr_auto] items-center gap-3 py-2 md:grid-cols-[1fr_auto_1fr]">
      ...
      <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-5 whitespace-nowrap md:flex lg:static lg:translate-x-0 lg:justify-self-center lg:gap-9">{links.map(([label, href]) => <a key={href} href={href} className="text-[13px] font-medium text-creaty-cream/65 transition-colors hover:text-creaty-white lg:text-sm">{label}</a>)}</nav>
      <div className="flex items-center justify-self-end gap-2"><CTAButton className="navbar-cta px-3 py-2 text-[11px] sm:px-4 sm:text-xs lg:px-5 lg:text-sm">Regístrate ahora</CTAButton><button type="button" aria-label={menuOpen ? "Cerrar menú" : "Abrir menú"} aria-expanded={menuOpen} onClick={() => setMenuOpen(v => !v)} className="grid h-10 w-10 place-items-center rounded-md border border-creaty-cream/15 text-creaty-cream md:hidden">{menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button></div>
    </div>
