...
function Navbar() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-[100] border-b transition-all duration-300 ${scrolled || menuOpen ? "border-creaty-cream/10 bg-creaty-black/95 shadow-[var(--shadow-nav)] backdrop-blur-xl" : "border-transparent bg-transparent"}`}>
    <div className="site-container grid grid-cols-[auto_1fr_auto] items-center gap-3 py-2">
      <Logo compact />
      <nav className="hidden items-center justify-center gap-7 lg:flex">{links.map(([label, href]) => <a key={href} href={href} className="text-sm font-medium text-creaty-cream/65 transition hover:text-creaty-cream">{label}</a>)}</nav>
      <div className="flex items-center justify-end gap-2"><CTAButton className="min-h-10 px-3 py-2 text-[11px] sm:px-4 sm:text-xs">Regístrate ahora</CTAButton><button type="button" aria-label={menuOpen ? "Cerrar menú" : "Abrir menú"} aria-expanded={menuOpen} onClick={() => setMenuOpen(v => !v)} className="grid h-10 w-10 place-items-center rounded-md border border-creaty-cream/15 text-creaty-cream lg:hidden">{menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button></div>
    </div>
...
function Hero() {
  ...
  return <section id="top" className="relative overflow-hidden bg-creaty-black pb-14 pt-32 text-creaty-cream md:pb-16 md:pt-36 lg:pb-20 lg:pt-40"><div className="hero-grid absolute inset-0 opacity-40"/><div className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-creaty-lime/50 to-transparent"/><div className="site-container"><motion.div initial={reduce?false:{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{duration:.8}} className="relative mx-auto text-center"><h1 className="type-hero mx-auto font-black tracking-normal">Más emprendimientos acompañados. <span className="text-creaty-lime">Menos tiempo</span> preparando cada mentoría.</h1><p className="type-support mx-auto mt-5 text-creaty-cream">Creaty centraliza los avances de tus emprendedores y te ayuda a llegar a cada sesión con el contexto y los próximos pasos claros, para que dediques tu tiempo a aportar valor.</p><div className="mt-8 flex flex-col items-center"><CTAButton>Regístrate ahora</CTAButton><span className="mt-3 text-sm font-medium text-creaty-cream">Y recibe gratis un diagnóstico como mentor para iniciar</span></div></motion.div></div></section>;
}
...
function Solucion(){return <Section id="solucion" light><SectionHeading title={<>Tu experiencia hace la mentoría. <span className="text-creaty-orange">Creaty</span> hace que llegues preparado.</>} description="Con Creaty conectas lo que ocurrió, lo que avanzó y lo que debería seguir para que tú puedas concentrarte en preguntar, analizar y orientar a tus emprendedores." descriptionClassName="text-creaty-black"/><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-6">{values.map(([Icon,t,d,c])=><article key={t} className="h-full rounded-lg border border-creaty-black/10 bg-background p-5 lg:p-6"><div className={`grid h-10 w-10 place-items-center rounded-md ${c}`}><Icon className="h-5 w-5"/></div><h3 className="type-card-title mt-4 font-extrabold">{t}</h3><p className="mt-2 text-sm leading-relaxed text-creaty-black/55">{d}</p></article>)}</div><ProcessMap/></Section>}
...
function FinalCTA(){return <Section><div className="mx-auto max-w-[1000px] text-center"><Target className="mx-auto h-10 w-10 text-creaty-orange"/><h2 className="type-section mx-auto mt-5 font-black">Tu siguiente mentoría empieza mucho antes de la sesión.</h2><p className="type-support mx-auto mt-4 text-creaty-cream">Regístrate, recibe gratis tu diagnóstico como mentor y sé de los primeros en conocer cómo Creaty puede ayudarte a acompañar más emprendimientos dedicando menos tiempo operativo a preparar cada sesión.</p><div className="mt-8"><CTAButton>Regístrate ahora</CTAButton></div></div></Section>}
