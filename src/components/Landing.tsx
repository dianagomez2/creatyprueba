...
function SectionHeading({ eyebrow, title, description, centered = false, eyebrowClassName = "text-creaty-orange", descriptionClassName = "" }: { eyebrow?: string; title: string; description?: string; centered?: boolean; eyebrowClassName?: string; descriptionClassName?: string }) {
  if (centered) return <div className="text-center"><div className="mx-auto max-w-[1000px]">{eyebrow && <p className={`text-xs font-bold uppercase ${eyebrowClassName}`}>{eyebrow}</p>}<h2 className={`type-section mx-auto font-black ${eyebrow ? "mt-3" : ""}`}>{title}</h2></div>{description && <p className={`type-support mx-auto mt-4 ${descriptionClassName}`}>{description}</p>}</div>;
  return <div className="section-heading"><div>{eyebrow && <p className={`text-xs font-bold uppercase ${eyebrowClassName}`}>{eyebrow}</p>}<h2 className={`type-section font-black ${eyebrow ? "mt-3" : ""}`}>{title}</h2></div>{description && <p className={`type-support ${descriptionClassName}`}>{description}</p>}</div>;
}
...
function Navbar() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-[100] border-b transition-all duration-300 ${scrolled || menuOpen ? "border-creaty-cream/10 bg-creaty-black/95 shadow-[var(--shadow-nav)] backdrop-blur-xl" : "border-transparent bg-transparent"}`}>
    ...
    {menuOpen && <nav className="border-t border-creaty-cream/10 bg-creaty-black lg:hidden"><div className="site-container py-4">{links.map(([label, href]) => <a key={href} href={href} onClick={() => setMenuOpen(false)} className="block border-b border-creaty-cream/10 py-3 text-sm text-creaty-cream/80 last:border-0">{label}</a>)}</div></nav>}
  </header>;
}
...
function DashboardMockup() {
  return <div className="relative mx-auto mt-10 md:mt-12">
...
function Hero() {
  ...
  return <section id="top" className="relative overflow-hidden bg-creaty-black pb-16 pt-36 text-creaty-cream md:pb-20 md:pt-40 lg:pb-24 lg:pt-44"><div className="hero-grid absolute inset-0 opacity-40"/><div className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-creaty-lime/50 to-transparent"/><div className="site-container"><motion.div initial={reduce?false:{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{duration:.8}} className="relative mx-auto text-center"><h1 className="type-hero mx-auto font-black tracking-normal">Más emprendimientos acompañados. <span className="text-creaty-lime">Menos tiempo</span> preparando cada mentoría.</h1><p className="type-support mx-auto mt-5 text-creaty-cream/70">Creaty centraliza los avances de tus emprendedores y te ayuda a llegar a cada sesión con el contexto y los próximos pasos claros, para que dediques tu tiempo a aportar valor.</p><div className="mt-8 flex flex-col items-center"><CTAButton>Regístrate ahora</CTAButton><span className="mt-3 text-xs text-creaty-cream/45">Y recibe gratis un diagnóstico como mentor para iniciar</span></div></motion.div></div></section>;
}

function DashboardSection(){return <Section light><SectionHeading eyebrow="Un solo lugar para acompañar mejor" title="Todo lo que necesitas saber antes de tu próxima mentoría." description="Avances, acuerdos, próximos pasos y recomendaciones de cada emprendimiento, organizados en un solo lugar." descriptionClassName="text-creaty-black/60"/><h3 className="type-card-title mt-10 text-left font-extrabold lg:mt-14">Llega con contexto. Empieza aportando valor desde el primer minuto.</h3><DashboardMockup/></Section>}
...
function Problema(){return <Section id="problema"><SectionHeading eyebrow="Por qué Creaty" title="Tu tiempo debería estar dedicado a mentorear, no reconstruyendo lo que pasó." description="Entre una sesión y otra, el contexto se dispersa, los compromisos se enfrían y preparar cada caso vuelve a consumir tiempo." descriptionClassName="text-creaty-cream/60"/><div className="mt-10 grid gap-4 md:grid-cols-2 lg:mt-14 lg:gap-6">{problems.map(([t,d],i)=><article key={t} className="h-full rounded-lg border border-creaty-cream/10 bg-creaty-black p-5 sm:p-6 lg:p-8"><span className={`text-3xl font-black ${i%2?"text-creaty-purple":"text-creaty-orange"}`}>0{i+1}</span><h3 className="type-card-title mt-4 font-bold">{t}</h3><p className="mt-3 text-sm leading-relaxed text-creaty-cream/55">{d}</p></article>)}</div><div className="mx-auto mt-12 flex max-w-2xl flex-col items-center gap-5 border-t border-creaty-cream/10 pt-8 text-center lg:mt-16"><p className="text-pretty font-semibold">Descubre qué puedes mejorar en tu forma de acompañar y recibe gratis tu diagnóstico como mentor.</p><CTAButton>Haz tu diagnóstico gratis</CTAButton></div></Section>}
...
function Diferencial() {
  return <Section id="diferencial" light>
    ...
    <div className="mt-10 grid gap-4 lg:mt-14 lg:grid-cols-2 lg:gap-6">
      <div className="h-full rounded-lg border border-creaty-black/10 bg-background p-5 sm:p-6 lg:p-8">
...
function Credibility(){return <Section light><div className="rounded-lg bg-creaty-purple/25 p-5 sm:p-6 lg:p-10"><div className="grid gap-6 lg:grid-cols-[auto_minmax(0,1fr)] lg:items-center lg:gap-10"><div className="grid h-16 w-16 place-items-center rounded-full bg-creaty-purple"><Users className="h-7 w-7"/></div><SectionHeading eyebrow="Diseñado junto a mentores empresariales" eyebrowClassName="text-creaty-black/50" title="Construido con quienes saben lo que significa acompañar emprendedores." description="Estamos desarrollando Creaty junto a mentores empresariales para convertir problemas reales de tu día a día en una herramienta que realmente ahorra tiempo y mejore tu acompañamiento." descriptionClassName="text-creaty-black/60"/></div></div></Section>}

function FinalCTA(){return <Section><div className="mx-auto max-w-[1000px] text-center"><Target className="mx-auto h-10 w-10 text-creaty-orange"/><h2 className="type-section mx-auto mt-5 font-black">Tu siguiente mentoría empieza mucho antes de la sesión.</h2><p className="type-support mx-auto mt-4 text-creaty-cream/65">Regístrate, recibe gratis tu diagnóstico como mentor y sé de los primeros en conocer cómo Creaty puede ayudarte a acompañar más emprendimientos dedicando menos tiempo operativo a preparar cada sesión.</p><div className="mt-8"><CTAButton>Regístrate ahora</CTAButton></div></div></Section>}
